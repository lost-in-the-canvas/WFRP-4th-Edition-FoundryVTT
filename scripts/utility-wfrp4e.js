...
class WFRP_Utility
{
  ...
  static handleTableClick(event)
  {
    ...
    if (event.button == 0)
    {
      let mytext = event.target.text || event.target.textContent;
      if (mytext.trim() == game.i18n.localize("ROLL.CritCast"))
      {
        html = WFRP_Tables.criticalCastMenu($(event.currentTarget).attr("data-table"));
      }

      else if (mytext.trim() == game.i18n.localize("ROLL.TotalPower"))
